...
(function () {
  ...
  function init() {
    ...
    // Instant 0ms startup: prepare markers & vascular map
    create3DSyringe();
    create3DVascularMap();
    renderQuickbar();
    ...
    // Show non-blocking sleek mini status for realistic model loader
    if (loadingOverlay) {
      loadingOverlay.classList.remove('hidden');
      loadingOverlay.classList.add('mini-mode');
    }
    ...
    function applyLoadedGLTF(gltf) {
      ...
      headModel.traverse(function (node) {
        if (node.isMesh) {
          ...
          // Disable heavy transparent eye reflection meshes that kill mobile frame rate
          if (node.name.includes('EyeScleraReflect') || node.name.includes('MeniscusEye')) {
            node.visible = false;
            return;
          }

          if (node.material) {
            // Memory & VRAM optimization: disable mipmap bloat for 40% memory savings
      ...
      if (size.y > 0.001) {
        ...
        if (Math.abs(size.y - targetHeight) > 0.5) {
